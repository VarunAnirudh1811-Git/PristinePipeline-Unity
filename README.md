...
\- Optionally use filename patterns (e.g. `T\_\*` for textures) for fine-grained control
...
\- Enforce or warn on naming conventions (e.g. `SM\_`, `SK\_`, `P\_`)
...
&#x20; - \*\*Name Pattern\*\* \*(optional)\* — a wildcard filter on the filename, e.g. `T\_\*` to only match textures named with the `T\_` prefix
...
&#x20; - \*\*Name Pattern\*\* — a wildcard that matches FBX filenames, e.g. `SM\_\*` for static meshes or `SK\_\*` for skeletal meshes
...
\- Enable \*\*Enforce Naming Convention\*\* to block any FBX import whose filename doesn't start with one of your defined valid prefixes (e.g. `SM\_`, `SK\_`, `P\_`)
